...
function diffusion_grt(C,R,Tinput,Pinput,dtdiff,ndim,NBC,nrest)
    ...
    while time<dtdiff
        ...
        for it_n = 1:nit
            ...
            #Create sparse blocks for the LHS matrix------------------
            L_blocks = Matrix{SparseMatrixCSC{Float64,Int}}(undef, 3, 3)
            block_idx = 1
            for i in 1:3
                for j in 1:3
                    D = D_blocks[block_idx][1]
                    add_identity = D_blocks[block_idx][2]
                    Dl = D[1:end-1]
                    Dr = D[2:end]
                    ld = -[Dl .* S .* xL; 0]
                    if add_identity
                        md = [0; (1.0 .* xC + Dl .* S .* xL + Dr .* S .* xR); 0]
                    else
                        md = [0; (Dl .* S .* xL + Dr .* S .* xR); 0]
                    end
                    ud = -[0; Dr .* S .* xR]
                    L_blocks[i, j] = spdiagm(-1 => ld, 0 => md, 1 => ud)
                    block_idx += 1
                end
            end
            L11B, L12B, L13B = L_blocks[1,1], L_blocks[1,2], L_blocks[1,3]
            L21B, L22B, L23B = L_blocks[2,1], L_blocks[2,2], L_blocks[2,3]
            L31B, L32B, L33B = L_blocks[3,1], L_blocks[3,2], L_blocks[3,3]
            #Assemble Matrix--------------------------------
            LHS  = [L11B L12B L13B;
                    L21B L22B L23B;
                    L31B L32B L33B]
